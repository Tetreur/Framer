...
ARGV.each do |x|
  base = `basename #{x}`
  prefixes = %w(29.7x29.7_ 30x30_ 40x40_ 42x42_ 50x50_ 59.4x59.4_ 60x60_ 65x65_ 70x70_ 75x75_ 80x80_ 90x90_ 100x100_) # TODO: Rajouter  120x120_ 140x140_
  #prefixes = %w(30x30_)
...
def homothetie_calc
  ...
  Dir.glob('FRAME/*.jpg').each do |f|
    ...
    size_to_resizes[size].each do |new_size|
      ...
      puts "Creating mask for #{new_name}"
      system("convert -size 1000x1000 xc:black -fill white -draw  'polygon 120,120 120,880 880,880 880,120' #{f[0..-5]}_MASK.jpg")
